...
### 5. 항공편 표시 개선
- **편명 표시 형식 변경**: "7C | 1801" → "제주항공 7C1801" 형식으로 개선
- **정규식 버그 수정**: `/^([A-Z0-9]{2})(\d+)$/i` → `/^([A-Z][A-Z0-9]|[0-9][A-Z])(\d+)$/i`
  - 순수 숫자 "1801"이 잘못 매칭되던 문제 해결 (문자 포함 필수로 변경)
- **수동 입력 섹션 제거**: 가는 편/오는 편 수동 입력 필드 삭제 (티켓 업로드 전용으로 단순화)
- **항공사 코드 매핑 추가**: 7C(제주항공), KE(대한항공), OZ(아시아나) 등 20+ 항공사 지원
