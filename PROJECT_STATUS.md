...
- **시스템 안정화**: 렌더링 오류(removeChild) 해결 및 타입 안정성 강화 (PlannerData interface)
...
- **OCR 파싱 정밀도 향상**: 아고다 바우처, 특정 항공사 티켓 등 파싱이 안 되는 케이스별 로직 보강
...
- **도구 사용 규칙**:
  1. **브라우저 도구 제한**: 토큰 소모 방지를 위해 브라우저 도구(`browser_subagent`) 사용 전 반드시 사용자 승인을 득할 것. (상세 내역은 `.agent/rules/BEHAVIOR_RULES.md` 참조)

**마지막 작업**: 브라우저 도구 복구 테스트 및 도구 사용 승인 규칙 수립
**다음 작업 예정**: OCR 파싱 정확도 개선 (추출 실패 케이스 분석 및 보강)
