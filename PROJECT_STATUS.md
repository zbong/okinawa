...
#### 📝 계획(플래닝) 단계 특화
- **일행과 함께 짜는 실시간 동시 편집 (Multiplayer)**: Supabase를 활용하여 링크 하나로 일행과 동시에 여행 일정을 작성하고 피드백을 주고받는 기능.
- **"이 날 축제가 있어요!" 일정 기반 이벤트 자동 추천**: 사용자가 입력한 여행 날짜와 지역에 맞춰 현지 마츠리(축제), 이벤트 등의 알림을 제공하는 기능.
- **AI 대화형 플래닝 챗봇 (Step-by-step Helper)**: 플래너 입력창 옆에 챗봇을 띄워, "숙소는 북부의 가성비 좋은 곳" 등을 요청하면 AI가 폼에 데이터를 자동 입력해 주는 기능.
- **예상 여행 경비 사전 계산기 (Budget Estimator)**: 선택한 여행 스타일, 렌터카 유무 등을 기반으로 러프한 1인당 예상 경비를 가이드 작성 완료 전에 브리핑.
- **드래그 앤 드롭 일정 수정 (Itinerary Builder)**: AI가 자동 생성해 준 일정을 타임라인 위에서 마우스 드래그로 손쉽게 순서를 바꾸고 확정할 수 있는 UI 제공.
