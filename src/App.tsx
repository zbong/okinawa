...
const App: React.FC = () => {
        ...
        try {
            ...
            const resolveAirportName = (code: string | undefined) => {
                ...
                const map: Record<string, string> = {
                    'ICN': '인천국제공항 (ICN)', 'GMP': '김포국제공항 (GMP)',
                    'KIX': '간사이국제공항 (KIX)', 'NRT': '나리타국제공항 (NRT)', 'HND': '하네다공항 (HND)', 'FUK': '후쿠오카공항 (FUK)', 'CTS': '신치토세공항 (CTS)', 'OKA': '나하공항 (OKA)',
                    'CJU': '제주국제공항 (CJU)', 'PUS': '김해국제공항 (PUS)', 'TAE': '대구국제공항 (TAE)', 'CJJ': '청주국제공항 (CJJ)',
                    'MWX': '무안국제공항 (MWX)', 'YNY': '양양국제공항 (YNY)'
                };
                if (code.length === 3 && code === code.toUpperCase()) return map[code] || code;
                return code;
            };
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (parsed.type === 'flight') {
                    ...
                    if (parsed.flight?.departureAirport) updates.departurePoint = resolveAirportName(parsed.flight.departureAirport);
                    if (parsed.flight?.arrivalAirport) updates.entryPoint = resolveAirportName(parsed.flight.arrivalAirport);
            ...
            // Post-processing for Flight Round Trip Logic
            const flightResults = newAnalyzedFiles.filter(f => f.parsedData?.type === 'flight').map(f => f.parsedData);
            if (flightResults.length > 0) {
                ...
                if (dep.flight?.departureAirport) depUpdates.departurePoint = resolveAirportName(dep.flight.departureAirport);
                if (dep.flight?.arrivalAirport) depUpdates.entryPoint = resolveAirportName(dep.flight.arrivalAirport);
                if (dep.flight?.departureDate) depUpdates.startDate = dep.flight.departureDate;
                if (dep.flight?.departureTime) depUpdates.departureTime = dep.flight.departureTime;
                if (dep.flight?.arrivalTime) depUpdates.arrivalTime = dep.flight.arrivalTime;
                if (dep.flight?.airline) depUpdates.airline = dep.flight.airline;
                if (dep.flight?.flightNumber) depUpdates.flightNumber = dep.flight.flightNumber;
                ...
                if (flightResults.length >= 2) {
                    ...
                    if (ret.flight?.airline) retUpdates.returnAirline = ret.flight.airline;
                    if (ret.flight?.flightNumber) retUpdates.returnFlightNumber = ret.flight.flightNumber;
                    if (ret.flight?.departureDate) retUpdates.endDate = ret.flight.departureDate; // Return flight date is EndDate
                    if (ret.flight?.departureTime) retUpdates.returnDepartureTime = ret.flight.departureTime;

                    // Return Flight Airports
                    if (ret.flight?.departureAirport) retUpdates.returnDeparturePoint = resolveAirportName(ret.flight.departureAirport);
                    if (ret.flight?.arrivalAirport) retUpdates.returnArrivalPoint = resolveAirportName(ret.flight.arrivalAirport);
    ...
    const fetchHotelsWithAI = async (destination: string) => {
        ...
        try {
            ...
            if (e.message?.includes('429') || e.toString().includes('429') || e.toString().includes('Resource exhausted')) {
    ...
    const validateAndAddPlace = async (name: string) => {
        ...
        const existingPlace = dynamicAttractions.find(p =>
        ...
        if (existingPlace) {
            if (selectedPlaceIds.includes(existingPlace.id)) {
                ...
                if (window.confirm(`"${existingPlace.name}"이(가) 추천 목록에 이미 있습니다.\n추가하시겠습니까?`)) {
                    setSelectedPlaceIds(prev => [...prev, existingPlace.id]);
                    showToast(`${existingPlace.name}이(가) 선택되었습니다.`);
            ...
            try {
                ...
                if (data && data.isValid) {
                    const newPlace = {
                        ...
                        longDesc: data.desc || '사용자가 직접 입력하여 추가한 장소입니다.',
                        rating: 0,
                        reviewCount: 0,
                        priceLevel: '',
                    ...
                    if (window.confirm(`"${name}"의 정확한 위치 정보를 찾을 수 없습니다.\n그래도 추가하시겠습니까?\n(위치 정보가 없으면 동선 최적화에서 제외될 수 있습니다.)`)) {
                        const newPlace = {
                            id: `manual-${Date.now()}`,
                            name: name,
                            category: 'custom',
                ...
                alert('장소 정보를 검증하는 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
            }
        } catch (e) {
                                    ...
                                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', paddingRight: '4px', flex: 1, textAlign: 'left' }}>
                                            ...
                                            {localStorage.getItem('trip_draft_v1') && (() => {
                                                const rawDraft = JSON.parse(localStorage.getItem('trip_draft_v1')!);
                                                // Handle legacy format (where draft was just plannerData)
                                                const draft = rawDraft.data ? rawDraft : { data: rawDraft, step: 0 };
                ...
                <AnimatePresence>
                    {isPlanning && (
                        ...
                        >
                            ...
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: plannerStep >= 3 ? 'flex-start' : 'center', alignItems: 'center', padding: '0 30px 60px', overflowY: 'auto' }}>
                                <div style={{ width: '100%', maxWidth: '700px', textAlign: 'center' }}>
                                    <AnimatePresence mode="wait">
                                        ...
                                        {plannerStep === 1 && (
                                            <motion.div key="planner-step-1" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '800px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '20px', justifyContent: 'center' }}>
                                                    {[1, 2, 3, 4, 5, 6].map(i => (
                                                        <div key={i} style={{ width: 10, height: 10, borderRadius: '50%', background: i === 1 ? 'var(--primary)' : 'rgba(255,255,255,0.1)', opacity: i < 1 ? 0.3 : 1 }} />
                                                    ))}
                                                </div>
                                                <h2 style={{ fontSize: '32px', fontWeight: 900, marginBottom: '8px' }}>여행의 기본 정보를 알려주세요</h2>
                                                <p style={{ opacity: 0.6, marginBottom: '32px' }}>언제, 어디로 떠나시나요?</p>
                                                ...
                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '24px', textAlign: 'left' }}>
                                                    ...
                                                    <div className="glass-card" style={{ padding: '20px', background: 'rgba(255,255,255,0.02)' }}>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                                                            <h3 style={{ fontSize: '17px', fontWeight: 800 }}>{calendarDate.getFullYear()}년 {calendarDate.getMonth() + 1}월</h3>
                                                            <div style={{ display: 'flex', gap: '8px' }}>
                                                                <button onClick={prevMonth} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', width: 30, height: 30, borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ChevronLeft size={16} /></button>
                                                                <button onClick={nextMonth} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', width: 30, height: 30, borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ChevronRight size={16} /></button>
                                                            </div>
                                                        </div>
                                        ...
                                        {plannerStep === 2 && (
                                            <motion.div key="planner-step-2" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '700px' }}>
                                                ...
                                                <h2 style={{ fontSize: '32px', fontWeight: 900, marginBottom: '30px', textAlign: 'center' }}>어떤 스타일의 여행인가요?</h2>

                                                <div style={{ marginBottom: '40px' }}>
                                                    <label style={{ display: 'block', fontSize: '14px', fontWeight: 700, marginBottom: '15px', color: 'var(--primary)', opacity: 0.8 }}>누구와 함께 가시나요?</label>
                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '15px', marginBottom: '20px' }}>
                                                        {[{ id: 'alone', label: '혼자서', icon: <User size={24} /> }, { id: 'couple', label: '연인과', icon: <Heart size={24} /> }, { id: 'friends', label: '친구와', icon: <Users size={24} /> }, { id: 'family', label: '가족과', icon: <Users size={24} /> }].map(item => (
                                                            <button key={item.id} onClick={() => setPlannerData({ ...plannerData, companion: item.id })} style={{ padding: '20px', borderRadius: '16px', border: plannerData.companion === item.id ? '2px solid var(--primary)' : '1px solid rgba(255,255,255,0.1)', background: plannerData.companion === item.id ? 'rgba(0,212,255,0.1)' : 'rgba(255,255,255,0.03)', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>{item.icon}<span style={{ fontWeight: 700, fontSize: '13px' }}>{item.label}</span></button>
                                                        ))}
                                                    </div>

                                                    <div style={{ padding: '20px', background: 'rgba(255,255,255,0.03)', borderRadius: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(255,255,255,0.1)' }}>
                                                        <div>
                                                            <div style={{ fontWeight: 700, color: 'white' }}>총 여행 인원</div>
                                                            <div style={{ fontSize: '13px', opacity: 0.6 }}>항공권 및 숙소 검색에 반영됩니다</div>
                                                        </div>
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                            <button onClick={() => setPlannerData({ ...plannerData, peopleCount: Math.max(1, (plannerData.peopleCount || 1) - 1) })} style={{ width: 40, height: 40, borderRadius: '12px', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Minus size={18} /></button>
                                                            <span style={{ fontSize: '20px', fontWeight: 800, width: 30, textAlign: 'center' }}>{plannerData.peopleCount || 1}</span>
                                                            <button onClick={() => setPlannerData({ ...plannerData, peopleCount: (plannerData.peopleCount || 1) + 1 })} style={{ width: 40, height: 40, borderRadius: '12px', background: 'var(--primary)', border: 'none', color: 'black', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}><Plus size={18} /></button>
                                                        </div>
                                                    </div>
                                                </div>

                                                <div style={{ marginBottom: '40px' }}>
                                                    <label style={{ display: 'block', fontSize: '14px', fontWeight: 700, marginBottom: '15px', color: 'var(--primary)', opacity: 0.8 }}>여행 속도는 어떠신가요?</label>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                                        {[{ id: 'slow', label: '여유롭게 (하루 2-3곳)', icon: <Clock size={20} /> }, { id: 'normal', label: '적당히 (하루 4-5곳)', icon: <Clock size={20} /> }, { id: 'fast', label: '빡빡하게 (하루 6곳+)', icon: <Clock size={20} /> }].map(item => (
                                                            <button key={item.id} onClick={() => setPlannerData({ ...plannerData, pace: item.id as any })} style={{ padding: '16px 20px', borderRadius: '16px', border: plannerData.pace === item.id ? '2px solid var(--primary)' : '1px solid rgba(255,255,255,0.1)', background: plannerData.pace === item.id ? 'rgba(0,212,255,0.1)' : 'rgba(255,255,255,0.03)', color: 'white', display: 'flex', alignItems: 'center', gap: 15, textAlign: 'left' }}>
                                                                {item.icon}
                                                ...
                                                <div style={{ display: 'flex', gap: '15px' }}>
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={!plannerData.companion || !plannerData.pace}
                                                        style={{
                                                            flex: 2, padding: '20px', borderRadius: '20px', border: 'none',
                                                            background: (plannerData.companion && plannerData.pace) ? 'var(--primary)' : 'rgba(255,255,255,0.1)',
                                                            color: (plannerData.companion && plannerData.pace) ? 'black' : 'rgba(255,255,255,0.3)',
                                                            fontWeight: 800
                                                        }}
                                                    >
                                                        다음 단계로 (교통편)
                                        ...
                                        {plannerStep === 3 && (
                                            <motion.div key="planner-step-3" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '700px', marginTop: '40px', paddingBottom: '100px', position: 'relative', zIndex: 10 }}>
                                                ...
                                                <h2 style={{ fontSize: '32px', fontWeight: 900, marginBottom: '10px', textAlign: 'center', color: 'white' }}>어떻게 오시나요?</h2>
                                                <p style={{ textAlign: 'center', opacity: 0.6, marginBottom: '20px' }}>교통편을 입력하면 일정에 자동으로 추가해 드립니다.</p>

                                                <div style={{ background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '12px', marginBottom: '30px', fontSize: '15px', textAlign: 'center', border: '1px solid rgba(255,255,255,0.1)' }}>
                                                    <span style={{ color: 'var(--primary)', fontWeight: 700 }}>{plannerData.startDate} ~ {plannerData.endDate}</span>
                                                ...
                                                <div style={{ textAlign: 'left', marginBottom: '30px', padding: '24px', background: 'rgba(255,255,255,0.03)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                                    ...
                                                    {plannerData.travelMode !== 'car' && (
                                                        <div style={{ marginBottom: '25px', display: 'flex', gap: '10px' }}>
                                                            ...
                                                            <div
                                                                ...
                                                                onDrop={(e) => {
                                                                    ...
                                                                    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
                                                                        handleFileAnalysis(Array.from(e.dataTransfer.files));
                                                                    }
                                                                }}
                                                                style={{
                                                                    flex: 1,
                                                            ...
                                                            >
                                                                <input type="file" ref={ticketFileInputRef} style={{ display: 'none' }} accept="image/*,.html,.htm,.pdf" onChange={handleTicketOcr} />
                                                                {isOcrLoading ? (
                                                                    <>
                                                                        <Loader2 size={24} className="animate-spin" color="var(--primary)" />
                                                                        <span style={{ fontSize: '13px', color: 'var(--primary)', fontWeight: 700 }}>티켓 분석 중...</span>
                                                                    </>
                                                                ) : (
                                                                    <>
                                                                        <Upload size={24} color="var(--primary)" />
                                                                        <div style={{ textAlign: 'center' }}>
                                                                            <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--primary)' }}>티켓/예매내역 업로드</div>
                                                                            <div style={{ fontSize: '11px', opacity: 0.5, marginTop: 4 }}>이미지나 PDF를 올리면 자동 입력됩니다</div>
                                                                        </div>
                                                                    </>
                                                                )}
                                                            </div>
                                                            ...
                                                            {analyzedFiles.length > 0 && (
                                                                <div style={{ marginTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                                                    <div style={{ fontSize: '11px', fontWeight: 700, opacity: 0.5, marginBottom: '2px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>업로드됨</div>
                                                                    {analyzedFiles.map((file, idx) => (
                                                                        <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(255,255,255,0.05)', borderRadius: '10px', fontSize: '13px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
                                                                                <FileText size={14} style={{ opacity: 0.7, color: 'var(--primary)' }} />
                                                                                <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '250px' }}>{file.name}</span>
                                                                                {file.status === 'loading' && <Loader2 size={12} className="animate-spin" />}
                                                                            </div>
                                                                            <button
                                                                                onClick={(e) => {
                                                                                    e.stopPropagation();
                                                                                    setDeleteConfirmModal({
                                                                                        ...
                                                                                        onConfirm: () => {
                                                                                            setAnalyzedFiles(prev => prev.filter((_, i) => i !== idx));
                                                                                            setDeleteConfirmModal({ isOpen: false, title: '', message: '', onConfirm: () => { } });
                                                                                        }
                                                                                    });
                                                                                }}
                                                                                style={{ background: 'rgba(255,0,0,0.1)', border: 'none', color: '#ff6b6b', cursor: 'pointer', padding: '6px', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                                                                title="파일 삭제"
                                                                            >
                                                                                <Trash2 size={14} />
                                                                            </button>
                                                            ...
                                                            {(() => {
                                                                ...
                                                                // Skyscanner
                                                                const sStart = plannerData.startDate ? plannerData.startDate.slice(2).replace(/-/g, '') : '';
                                                                const sEnd = plannerData.endDate ? plannerData.endDate.slice(2).replace(/-/g, '') : '';

                                                                let skyscannerUrl = 'https://www.skyscanner.co.kr';
                                                                if (plannerData.destination && (plannerData.destination.includes('오키나와') || plannerData.destination.toLowerCase().includes('okinawa'))) {
                                                                    ...
                                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '40%' }}>
                                                                        <a
                                                                            ...
                                                                            style={{
                                                                                ...
                                                                                background: 'rgba(255,255,255,0.05)', borderRadius: '12px', color: 'white', textDecoration: 'none', fontSize: '13px', fontWeight: 600
                                                                            }}
                                                                        >
                                                        ...
                                                        {plannerData.travelMode === 'plane' ? (
                                                            <>
                                                                ...
                                                                <div style={{ background: 'rgba(255,255,255,0.03)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                                                    <h4 style={{ color: '#60a5fa', marginBottom: '15px', fontWeight: 800, fontSize: '14px', display: 'flex', alignItems: 'center', gap: 8 }}>🛫 가는 편 (출국)</h4>
                                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                                                                        <input type="text" placeholder="항공사" value={plannerData.airline || ''} onChange={(e) => setPlannerData({ ...plannerData, airline: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        <input type="text" placeholder="편명 (예: KE001)" value={plannerData.flightNumber || ''} onChange={(e) => setPlannerData({ ...plannerData, flightNumber: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '10px' }}>
                                                                        <input type="text" placeholder="출발 공항 (ICN)" value={plannerData.departurePoint} onChange={(e) => setPlannerData({ ...plannerData, departurePoint: e.target.value })} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        <ArrowRight size={14} color="rgba(255,255,255,0.3)" />
                                                                        <input type="text" placeholder="도착 공항 (OKA)" value={plannerData.entryPoint === 'Direct Driving' ? '' : plannerData.entryPoint} onChange={(e) => setPlannerData({ ...plannerData, entryPoint: e.target.value })} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                    <div style={{ display: 'flex', gap: '5px' }}>
                                                                        <input type="date" value={plannerData.startDate || ''} onChange={(e) => setPlannerData({ ...plannerData, startDate: e.target.value })} style={{ flex: 3, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        <input type="time" value={plannerData.departureTime || ''} onChange={(e) => setPlannerData({ ...plannerData, departureTime: e.target.value })} style={{ flex: 2, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                </div>

                                                                <div style={{ background: 'rgba(255,255,255,0.03)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                                                    <h4 style={{ color: '#fbbf24', marginBottom: '15px', fontWeight: 800, fontSize: '14px', display: 'flex', alignItems: 'center', gap: 8 }}>🛬 오는 편 (귀국)</h4>
                                                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                                                                        <input type="text" placeholder="항공사" value={plannerData.returnAirline || ''} onChange={(e) => setPlannerData({ ...plannerData, returnAirline: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        <input type="text" placeholder="편명 (예: KE002)" value={plannerData.returnFlightNumber || ''} onChange={(e) => setPlannerData({ ...plannerData, returnFlightNumber: e.target.value })} style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '10px' }}>
                                                                        <input type="text" placeholder="출발 공항 (OKA)" value={plannerData.returnDeparturePoint || ''} onChange={(e) => setPlannerData({ ...plannerData, returnDeparturePoint: e.target.value })} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        <ArrowRight size={14} color="rgba(255,255,255,0.3)" />
                                                                        <input type="text" placeholder="도착 공항 (ICN)" value={plannerData.returnArrivalPoint || ''} onChange={(e) => setPlannerData({ ...plannerData, returnArrivalPoint: e.target.value })} style={{ flex: 1, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                    <div style={{ display: 'flex', gap: '5px' }}>
                                                                        <input type="date" value={plannerData.endDate || ''} onChange={(e) => setPlannerData({ ...plannerData, endDate: e.target.value })} style={{ flex: 3, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        <input type="time" value={plannerData.returnDepartureTime || ''} onChange={(e) => setPlannerData({ ...plannerData, returnDepartureTime: e.target.value })} style={{ flex: 2, padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                </div>
                                                            </>
                                                        ) : (
                                                            <>
                                                                ...
                                                                {/* Places */}
                                                                <div>
                                                                    <label style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '13px', fontWeight: 700, opacity: 0.8 }}>
                                                                        <span>출발지</span>
                                                                        {plannerData.departureCoordinates && <span style={{ fontSize: '10px', color: '#10b981' }}>✓ 위치 확인됨</span>}
                                                                    </label>
                                                                    <input type="text" placeholder="예: 출발지 입력" value={plannerData.departurePoint} onChange={(e) => setPlannerData({ ...plannerData, departurePoint: e.target.value })} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                </div>
                                                                ...
                                                                {plannerData.travelMode !== 'car' && (
                                                                    <div>
                                                                        <label style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', fontSize: '13px', fontWeight: 700, opacity: 0.8 }}>
                                                                            <span>도착지</span>
                                                                            {plannerData.entryCoordinates && <span style={{ fontSize: '10px', color: '#10b981' }}>✓ 위치 확인됨</span>}
                                                                        </label>
                                                                        <input type="text" placeholder={`예: ${plannerData.destination} 항구/터미널`} value={plannerData.entryPoint === 'Direct Driving' ? '' : plannerData.entryPoint} onChange={(e) => setPlannerData({ ...plannerData, entryPoint: e.target.value })} style={{ width: '100%', padding: '14px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                    </div>
                                                                )}
                                                                ...
                                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                                                                    <div>
                                                                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 700, opacity: 0.8 }}>출발 일시</label>
                                                                        <div style={{ display: 'flex', gap: '5px' }}>
                                                                            <input type="date" value={plannerData.startDate || ''} onChange={(e) => setPlannerData({ ...plannerData, startDate: e.target.value })} style={{ flex: 3, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                            <input type="time" value={plannerData.departureTime || ''} onChange={(e) => setPlannerData({ ...plannerData, departureTime: e.target.value })} style={{ flex: 2, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        </div>
                                                                    </div>
                                                                    <div>
                                                                        <label style={{ display: 'block', marginBottom: '8px', fontSize: '13px', fontWeight: 700, opacity: 0.8 }}>도착 일시</label>
                                                                        <div style={{ display: 'flex', gap: '5px' }}>
                                                                            <input type="date" value={plannerData.arrivalDate || plannerData.startDate || ''} onChange={(e) => setPlannerData({ ...plannerData, arrivalDate: e.target.value })} style={{ flex: 3, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                            <input type="time" value={plannerData.arrivalTime || ''} onChange={(e) => setPlannerData({ ...plannerData, arrivalTime: e.target.value })} style={{ flex: 2, padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(0,0,0,0.3)', color: 'white' }} />
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </>
                                                ...
                                                <div style={{ display: 'flex', gap: '15px' }}>
                                                    ...
                                                    }} style={{ flex: 1, padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'white', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
                                                        <Save size={20} />
                                        ...
                                        {plannerStep === 4 && (
                                            <motion.div key="planner-step-4" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '900px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '40px', justifyContent: 'center' }}>
                                                    {[1, 2, 3, 4, 5].map(i => (
                                                ...
                                                {isSearchingAttractions ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px', marginBottom: '20px', maxHeight: '500px', overflowY: 'auto', padding: '10px', textAlign: 'left' }}>
                                                            {dynamicAttractions
                                                                                ...
                                                                                <div style={{ flex: 1 }}>
                                                                                    <div style={{ display: 'flex', gap: '8px', marginBottom: '6px', alignItems: 'center' }}>
                                                                                        <span style={{ fontSize: '11px', padding: '4px 8px', borderRadius: '6px', background: 'rgba(255,255,255,0.1)', color: '#cbd5e1' }}>
                                                                                            {item.category === 'food' ? '식당' : item.category === 'cafe' ? '카페' : item.category === 'custom' ? '직접입력' : '관광'}
                                                                                        </span>
                                                                                        {item.priceLevel && (
                                                        ...
                                                        <div className="glass-card" style={{ padding: '20px', display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '40px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
                                                                ...
                                                                <input
                                                                    ...
                                                                    placeholder={isValidatingPlace ? "AI가 장소 정보를 확인 중입니다..." : "원하는 장소가 없다면 직접 입력하여 추가하세요 (예: 할머니 댁, 스타벅스 나하점)"}
                                                                    ...
                                                                    onKeyDown={async (e) => {
                                                                        if (e.key === 'Enter' && !isValidatingPlace) {
                                                                            ...
                                                                            if (name) {
                                                                                const success = await validateAndAddPlace(name);
                                                                                if (success) {
                                                                                    input.value = '';
                                                                                }
                                                                            }
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={async () => {
                                                                    const input = document.getElementById('custom-place-input') as HTMLInputElement;
                                                                    const name = input.value.trim();
                                                                    if (!name) return showToast('장소 이름을 입력해주세요.');
                                                                ...
                                                                style={{
                                                                    ...
                                                                    border: 'none',
                                                                    fontWeight: 800,
                                                                    cursor: isValidatingPlace ? 'wait' : 'pointer',
                                                                    display: 'flex', alignItems: 'center', gap: 6,
                                                ...
                                                <div style={{ display: 'flex', gap: '15px' }}>
                                                    <button onClick={() => setPlannerStep(3)} style={{ flex: 1, padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'white', fontWeight: 800 }}>이전 (교통편)</button>
                                                    <button onClick={() => {
                                                        const draft = {
                                                            step: 4,
                                                            data: plannerData,
                                                    ...
                                                    <button onClick={() => {
                                                        ...
                                                        setPlannerStep(5);
                                                    }} style={{ flex: 2, padding: '20px', borderRadius: '20px', border: 'none', background: 'var(--primary)', color: 'black', fontWeight: 800 }}>다음 단계로 (숙소)</button>
                                                </div>
                                            </motion.div>
                                        )}

                                        {plannerStep === 5 && (
                                            <motion.div key="planner-step-5" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '800px', textAlign: 'left' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: '40px', justifyContent: 'center' }}>
                                                    {[1, 2, 3, 4, 5].map(i => (
                                                ...
                                                <p style={{ opacity: 0.6, marginBottom: '32px', textAlign: 'center' }}>선택한 장소들을 고려하여 숙소를 정해보세요.</p>

                                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '30px' }}>
                                                    {/* AI Recommendation */}
                                                    <div className="glass-card" style={{ padding: '30px', border: '2px dashed rgba(0,212,255,0.3)', background: 'rgba(0,212,255,0.02)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 15, cursor: isSearchingHotels ? 'wait' : 'pointer', transition: 'all 0.2s' }}
                                                        onClick={() => !isSearchingHotels && fetchHotelsWithAI(plannerData.destination)}
                                                        onMouseEnter={(e) => !isSearchingHotels && (e.currentTarget.style.background = 'rgba(0,212,255,0.05)')}
                                                        onMouseLeave={(e) => !isSearchingHotels && (e.currentTarget.style.background = 'rgba(0,212,255,0.02)')}
                                                    >
                                                        {isSearchingHotels ? (
                                                            <>
                                                                <Loader2 size={32} className="animate-spin" color="var(--primary)" />
                                                                <div style={{ textAlign: 'center' }}>
                                                                    <div style={{ fontWeight: 800, fontSize: '16px', color: 'var(--primary)' }}>AI 분석 중...</div>
                                                                    <div style={{ fontSize: '12px', opacity: 0.6, marginTop: 4 }}>최적의 숙소를 찾고 있습니다</div>
                                                                </div>
                                                            </>
                                                        ) : (
                                                            <>
                                                    ...
                                                    <div style={{ display: 'grid', gridTemplateRows: '1fr 1fr', gap: '10px' }}>
                                                        <a href={`https://www.agoda.com/search?city=${plannerData.destination}`} target="_blank" rel="noreferrer" className="glass-card"
                                                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, textDecoration: 'none', color: 'white', background: 'rgba(255,255,255,0.05)', borderRadius: '16px' }}>
                                                            <Hotel size={20} /> <span style={{ fontWeight: 700 }}>아고다에서 찾기</span>
                                                        </a>
                                                        <a href={`https://www.visitokinawa.jp/`} target="_blank" rel="noreferrer" className="glass-card"
                                                            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, textDecoration: 'none', color: 'white', background: 'rgba(255,255,255,0.05)', borderRadius: '16px' }}>
                                                            <Globe size={20} /> <span style={{ fontWeight: 700 }}>비짓 오키나와</span>
                                                        </a>
                                                    </div>
                                                </div>

                                                {/* Manual Entry Form */}
                                                <div className="glass-card" style={{ padding: '30px', display: 'flex', flexDirection: 'column', gap: 15, marginBottom: '30px', border: '1px solid rgba(255,255,255,0.1)' }}>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '5px' }}>
                                                        <Hotel size={24} color="var(--primary)" />
                                                        <span style={{ fontWeight: 800, fontSize: '18px' }}>숙소 직접 등록</span>
                                                    </div>
                                                    <input id="acc-name" type="text" placeholder="숙소 이름 (예: 힐튼 나하)" style={{ width: '100%', padding: '16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'white', fontSize: '15px' }} />
                                                    <div style={{ display: 'flex', gap: 10 }}>
                                                        <div style={{ flex: 1 }}>
                                                            <label style={{ display: 'block', fontSize: '12px', marginBottom: '4px', opacity: 0.7 }}>체크인</label>
                                                            <input id="acc-start" type="date" defaultValue={plannerData.startDate} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'white', fontSize: '14px' }} />
                                                        </div>
                                                        <div style={{ flex: 1 }}>
                                                            <label style={{ display: 'block', fontSize: '12px', marginBottom: '4px', opacity: 0.7 }}>체크아웃</label>
                                                            <input id="acc-end" type="date" defaultValue={plannerData.endDate} style={{ width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'white', fontSize: '14px' }} />
                                                        </div>
                                                    </div>
                                                    <button
                                                        onClick={() => {
                                                            ...
                                                            if (!name || !start || !end) return showToast('숙소 이름과 날짜를 모두 입력해 주세요.');

                                                            const newAcc = { name, startDate: start, endDate: end };
                                                            setPlannerData({ ...plannerData, accommodations: [...plannerData.accommodations, newAcc] });
                                                            (document.getElementById('acc-name') as HTMLInputElement).value = '';
                                                            showToast('숙소가 목록에 추가되었습니다.');
                                                        }}
                                                        style={{ width: '100%', padding: '16px', borderRadius: '12px', background: 'var(--primary)', color: 'black', border: 'none', fontWeight: 800, cursor: 'pointer', marginTop: '10px' }}
                                                    >
                                                        <Plus size={18} style={{ verticalAlign: 'middle', marginRight: '5px' }} /> 목록에 추가
                                                    </button>
                                                </div>
                                                    ...
                                                    <div style={{ marginBottom: '30px' }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '10px' }}>
                                                            {recommendedHotels.map((h, i) => (
                                                                <div key={i} onClick={() => { const nameInput = document.getElementById('acc-name') as HTMLInputElement; if (nameInput) nameInput.value = h.name; showToast(`${h.name}를 선택했습니다. 날짜를 확인하고 추가하세요.`); }} className="glass-card" style={{ minWidth: '200px', padding: '15px', background: 'rgba(255,255,255,0.05)', cursor: 'pointer', border: '1px solid rgba(255,255,255,0.1)' }}>
                                                ...
                                                {plannerData.accommodations.length > 0 && (
                                                    <div style={{ marginBottom: '30px' }}>
                                                        <h4 style={{ fontSize: '16px', fontWeight: 800, marginBottom: '15px', paddingLeft: '5px' }}>등록된 숙소 ({plannerData.accommodations.length})</h4>
                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                                            {plannerData.accommodations.map((acc: any, idx: number) => (
                                                                <div key={idx} className="glass-card" style={{ padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }}>
                                                                    <div>
                                                ...
                                                <div style={{ display: 'flex', gap: '15px' }}>
                                                    <button onClick={() => setPlannerStep(4)} style={{ flex: 1, padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'white', fontWeight: 800 }}>이전 (장소 선택)</button>
                                                    <button onClick={() => {
                                                        const draft = {
                                                            step: 5,
                                                            data: plannerData,
                                                    ...
                                                    <button onClick={() => {
                                                        ...
                                                        const draft = { data: plannerData, step: 5, selectedIds: selectedPlaceIds, updated: Date.now() };
                                                        localStorage.setItem('trip_draft_v1', JSON.stringify(draft));

                                                        // Open Review Modal
                                                        setIsReviewModalOpen(true);
                                                    }} style={{ flex: 2, padding: '20px', borderRadius: '20px', border: 'none', background: 'var(--primary)', color: 'black', fontWeight: 900, fontSize: '18px' }}>최종 검토 및 생성</button>
                                                </div>
                                            </motion.div>
                                        )}
                                        ...
                                        {plannerStep === 7 && (
                                            <motion.div key="planner-step-9" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '600px', textAlign: 'left' }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '15px' }}>
                                                    <button onClick={() => setPlannerStep(5)} style={{ flex: 1, padding: '20px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: 'white', fontWeight: 800 }}>장소 수정</button>
                                                    <button onClick={() => {
